...
const AccountCreationSteps = () =>{
  ...
  return(
    <>
      <div className="w-full font-poppins py-4">
          ...
          {steps.map((step, index) => (
            ...
            >
              <div className="flex justify-center items-center text-gray-500">
                <span className="border-2 border-gray-500 rounded-full md:h-12 md:w-12 h-10 w-10 flex items-center justify-center bg-white">
                  {step.icon}
                </span>
              </div>
              <p className="text-center mt-2 text-xs md:text-sm text-gray-500 px-1">
                {step.title}
              </p>
            </div>
          ))}
